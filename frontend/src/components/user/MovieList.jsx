...
export default function MovieList({ title, movies = [] }) {
  ...
  return (
    <div>
      {title ? (
        <h1 className="text-2xl dark:text-custom-gray text-primary font-semibold font-mono mb-5">
          {title}
        </h1>
      ) : null}
      <GridContainer>
...
const ListItem = ({ movie }) => {
  ...
  return (
    <Link to={"/movie/" + id}>
      ...
      />
      <h1
        className="text-lg dark:text-custom-gray text-primary font-semibold font-mono"
        title={title}
      >
        {trimTitle(title)}
      </h1>
