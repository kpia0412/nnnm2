...
export default function HeroSlideshow() {
  ...
  const startSlideShow = () => {
    intervalId = setInterval(() => {
      newTime = Date.now();
      const differenceTime = newTime - previousTime;
  ...
  return (
    <div className="w-full flex">
      ...
      <div className="w-1/5 md:block hidden space-y-3 px-3">
        <h1 className="font-semibold font-mono text-2xl text-primary dark:text-custom-gray">
          Up Next
        </h1>
...
const SlideShowController = ({ onNextClick, onPrevClick }) => {
  const btnClass =
    "bg-primary rounded border-2 text-custom-gray text-xl p-2 outline-none";
  return (
...
const Slide = forwardRef((props, ref) => {
  ...
  return (
    ...
    >
      ...
      {title ? (
        <div className="absolute inset-0 flex flex-col justify-end py-3 bg-gradient-to-t from-custom-gray via-transparent dark:from-primary dark:via-transparent font-mono">
          <h1 className="font-semibold text-4xl dark:text-highlight-dark text-highlight font-mono">
            {title}
          </h1>
        </div>
      ) : null}
