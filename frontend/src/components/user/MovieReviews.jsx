...
export default function MovieReviews() {
  ...
  const findProfileOwnersReview = () => {
    ...
    if (!matched) {
      return updateNotification("error", "You do not have any reviews!");
    }
    ...
    <div className="dark:bg-primary bg-custom-gray min-h-screen pb-10">
      <Container className="xl:px-0 px-2 py-8">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-semibold dark:text-custom-gray text-secondary">
            <span className="text-primary dark:text-custom-gray font-mono">
              Reviews for:
            </span>{" "}
            {movieTitle}
          </h1>
          ...
          {profileId ? (
            <CustomButtonLink
              label={profileOwnersReview ? "View All" : "My Reviews"}
        ...
        {profileOwnersReview ? (
          <div>
            ...
            <div className="flex space-x-3 dark:text-custom-gray text-primary text-xl p-3">
...
const ReviewCard = ({ review }) => {
  ...
  return (
    <div className="flex space-x-3">
      <div className="flex items-center justify-center w-14 h-14 rounded-full bg-light-subtle dark:bg-dark-subtle text-custom-gray text-xl select-none">
        {getNameInitial(owner.name)}
      </div>
      <div>
        <h1 className="dark:text-custom-gray text-secondary font-semibold text-lg">
          {owner.name}
        </h1>
